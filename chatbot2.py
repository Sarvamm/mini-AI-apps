...
st.set_page_config(page_title="Gemma 3 Chatbot", layout="wide", initial_sidebar_state="expanded")
...
def get_gemma_response(prompt, system_prompt="You are a helpful assistant.", model=OLLAMA_MODEL):
    try:
...
def get_file_content(file_path, file_type, max_length=10000):
    try:
        if file_type.startswith('text/') or file_path.endswith(('.txt', '.md', '.py', '.js', '.html', '.css', '.json', '.csv')):
    ...
    if files:
        ...
        for file_id in files:
            if file_id in st.session_state.uploaded_files:
                ...
                if file_info['type'].startswith('text/') or file_info['name'].endswith(('.txt', '.md', '.py', '.js', '.html', '.css', '.json', '.csv')):
                    content = get_file_content(file_info['temp_path'], file_info['type'])
                ...
                # For other files, just mention them
                    ...
                    file_contents.append(f"File: {file_info['name']} ({file_info['type']})")
...
with st.sidebar:
    ...
    if ollama_available:
        ...
        if available_models:
            ...
            st.caption("No models found in Ollama. Make sure you've pulled the models first.")
            if st.button("Pull Gemma 3"):
                st.info("Attempting to pull gemma3 model... This might take a while.")
                try:
                    response = requests.post(f"{OLLAMA_API_BASE}/pull", json={"name": "gemma3"})
                    if response.status_code == 200:
                        st.success("Successfully initiated model pull. This might take a while to complete.")
                    else:
        ...
        st.info("Please make sure Ollama is running and accessible at http://localhost:11434")
    ...
    st.session_state.system_prompt = st.text_area(
        "System Instructions", 
        value=st.session_state.get("system_prompt", "You are a helpful AI assistant powered by Gemma 3. Answer questions clearly and helpfully."),
        height=100
    )
        ...
        if "files" in message and message["files"]:
            ...
            for file_id in message["files"]:
                if file_id in st.session_state.uploaded_files:
                    ...
                    elif file_info['type'].startswith('text/') or file_info['name'].endswith(('.txt', '.md', '.py')):
                        with open(file_info['temp_path'], 'r', errors='replace') as f:
                            content = f.read()
                            st.code(content[:1000] + ('...' if len(content) > 1000 else ''))

# File uploader
uploaded_files = st.file_uploader("Upload files to include in your message", 
...
if uploaded_files:
    ...
    for uploaded_file in uploaded_files:
        ...
        st.write(f"- {file_info['name']} ({file_info['type']}, {file_info['size']} bytes)")
...
with col1:
    st.text_input("Type your message here...", key="user_input", on_change=on_message_submit)
with col2:
    if st.button("Send"):
        on_message_submit()
